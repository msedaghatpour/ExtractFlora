...
today = date.today()
today = today.strftime("%Y%b%d") # format YYYmonDD
...
def get_center(vol_char_df, page_num, bias = 30):
...
for vol_char_df ,vol_index, doc in all_vol_data_col_num: 
    ...
    vol_char_df['epithet_index_pat_match'] = (vol_char_df['page_num'].isin(vol_index)) & (vol_char_df.apply(species_match, axis = 1))
    
    for page_num in tqdm(vol_index, desc="Finding Columns"):
        center_x0 = get_center(vol_char_df, page_num)
        #find center based on x0 coordinate of each line
        vol_char_df['col_num'] = vol_char_df['line_bbox'].apply(lambda coords : get_col(coords, center_x0)) 
...
all_vol_data_flagg_strict_match = [(vol1_char_df, vol1_index, vol1_doc, "strict_match_vol1"),
...
for vol_char_df, vol_index, vol_doc, output_name in all_vol_data_flagg_strict_match: 
    ...
    genus_flag_list = []
    epithet_flag_list = []
    for page_num in tqdm(vol_index, desc = "Finding strict matches"):
        ...
        if not os.path.exists(f"output{today}/index_checkpoint"):
            os.makedirs(f"output{today}/index_checkpoint")
    
    image_list[0].save(f'output{today}/index_checkpoint/'+output_name+'.pdf' ,save_all=True, append_images=image_list[1:])    
...
# Matching specific epithets based on coordinates
for vol_char_df, vol_index in all_vol_data_coord_match: 
    ...
    for page_num in tqdm(vol_index, desc = "Matching specific epithets"):
...
# Drawing bboxes for specific epithets
for vol_char_df, vol_index, doc, output_name in all_vol_data_epithet_coord_match_test: 
    #for each volume 
    image_list = []
    
    for page_num in tqdm(vol_index,  desc = "Printing PDF"):
...
for vol_char_df, vol_index in all_vol_data_coord_match: 
    ...
    for page_num in tqdm(vol_index, desc = "Matching generic epithets"):
...
# %%
# new cols
vol1_char_df['genus_coord_match'] = vol1_char_df.apply(genus_coord_match, axis = 1)
vol1_char_df['species_coord_match'] = vol1_char_df.apply(species_coord_match, axis = 1)

vol2_char_df['genus_coord_match'] = vol2_char_df.apply(genus_coord_match, axis = 1)
vol2_char_df['species_coord_match'] = vol2_char_df.apply(species_coord_match, axis = 1)

vol3_char_df['genus_coord_match'] = vol3_char_df.apply(genus_coord_match, axis = 1)
vol3_char_df['species_coord_match'] = vol3_char_df.apply(species_coord_match, axis = 1)
...
for vol_char_df, vol_index, doc, output_name in all_vol_data_GE_potential_match_test: 
    ...
    for page_num in tqdm(vol_index, desc = "Printing PDF"):
        ...
        epithet_db = vol_char_df[(vol_char_df['page_num'] == page_num) 
                                & (vol_char_df['species_coord_match'] == True)
                                ].loc[:,~vol_char_df.columns.isin(["char_num", "char", "char_origin",	"char_bbox"])
                                ].drop_duplicates()
...
vol1_char_df.loc[target_index, 'species_coord_match'] = False
vol1_char_df.loc[target_index, 'genus_coord_match'] = True
...
for vol_char_df, vol_index in all_vol_data_coord_match: 
    ...
    for page_num in tqdm(vol_index,  desc = "Doing something"):
        ...
        mean_right_epithet = vol_char_df[(vol_char_df["page_num"] == page_num) & (vol_char_df["col_num"] == 1) & (vol_char_df["species_coord_match"] == True)]["word_bbox"].apply(lambda x : x[0]).mean()
        ...
        if math.isnan(mean_right_genus):
            ...
            mean_right_epithet_all = vol_char_df[(vol_char_df["col_num"] == 1) & (vol_char_df["species_coord_match"] == True)]["word_bbox"].apply(lambda x : x[0]).mean()
...
# def author_infra_coord_match(row):
# word = row['word']
...
def author_infra_coord_match(word):
    lower_word = word.lower()
    latin_connectives = r"^\s?et[\s|.]?$|^\s?in[\s|.]?$|^\s?non[\s|.]?$|^\s?&[\s|.]?$|^\s?er[\s|.]?$|^\s?nec[\s|.]?$|^\s?mult[\s|.]?$|^\s?ex[\s|.]?$|^\s?fil[\s|.]?$"
    infra_symbols = r"^var[\s|.|\b]?$|^subsp[\s|.|\b]?$|^ssp[\s|.|\b]?$|^spp[\s|.|\b]?$|^x[\s|.|\b]?$|^×[\s|.|\b]?$"
    is_latin_connectives = re.search(latin_connectives, word) != None
...
for vol_char_df, _ in all_vol_data_coord_match:
    vol_char_df["potential_infra_match"] = (vol_char_df['word'].apply(has_infra_term)) | \
                                           ((vol_char_df["infra_coord_match"] == True) & (vol_char_df['word'].apply(author_infra_coord_match) == False))
...
for vol_char_df, vol_index, doc, output_name in all_vol_dat_infra_match_test: 
    ...
    for page_num in tqdm(vol_index, desc = "Printing PDF"):
        ...
        with_infra_symbols = vol_char_df[(vol_char_df['page_num'] == page_num) &
                                         (vol_char_df['infra_coord_match'] == True) & 
                                         (vol_char_df['word'].apply(has_infra_term) == True)
                                        ].loc[:,~vol_char_df.columns.isin(["char_num", "char", "char_origin",	"char_bbox"])
                                        ].drop_duplicates()
...
# Finding columns again? 
for vol_char_df ,vol_index, vol_doc in all_vol_data_col_num: 
    #for each volume check if genus pattern / epithet pattern exists within the index part of the book
    for page_num in tqdm(vol_index, desc = "Finding columns"):
        center_x0 = get_center(vol_char_df, page_num, - 30)
        #find center based on x0 coordinate of each line
        vol_char_df['col_num_for_PN'] = vol_char_df['line_bbox'].apply(lambda coords : get_col(coords, center_x0)) 
...
# %%
vol1_char_df[(vol1_char_df['potential_infra_match'] == True) & (vol1_char_df['word'].apply(has_infra_term) == False)][["index_page_num", "word"]].drop_duplicates()

# %%
vol2_char_df[(vol2_char_df['potential_infra_match'] == True) & (vol2_char_df['word'].apply(has_infra_term) == False)][["index_page_num", "word"]].drop_duplicates()

# %%
vol3_char_df[(vol3_char_df['potential_infra_match'] == True) & (vol3_char_df['word'].apply(has_infra_term) == False)][["index_page_num", "word"]].drop_duplicates()

# %%
remove_infra_index = [1566359, 1570483, 1578491, 1581443, 1582167, 1594835, 1598587]
#temp_df_hard_code_infra = vol3_char_df[(vol3_char_df['potential_infra_match'] == True) & (vol3_char_df['word'].apply(has_infra_term) == False)][["index_page_num", "pruned_word"]].drop_duplicates()
...
for i in set_epithet_index:
    vol3_char_df.loc[i : get_index_end(vol3_char_df, i),'species_coord_match'] = True

# %%
# ock keeps needing to be pruned multiple times ....??? not sure why ugh
vol3_char_df[(vol3_char_df['potential_infra_match'] == True) & (vol3_char_df['word'].apply(has_infra_term) == False)][["index_page_num", "word"]].drop_duplicates()
...
# %%
vol1_char_df[(vol1_char_df['species_coord_match'] == True) & (vol1_char_df['word'].apply(author_coord_match))][["index_page_num", "word"]].drop_duplicates()
...
vol2_char_df[(vol2_char_df['species_coord_match'] == True) & (vol2_char_df['word'].apply(author_coord_match))][["index_page_num", "word"]].drop_duplicates()

# %% [markdown]
# CHECKED vol3 

# %%
vol3_char_df[(vol3_char_df['species_coord_match'] == True) & (vol3_char_df['word'].apply(author_coord_match))][["index_page_num", "word"]].drop_duplicates()
...
for i in not_epithet_index_list:
    vol3_char_df.loc[i : get_index_end(vol3_char_df, i),'species_coord_match'] = False
...
# %%
vol1_char_df[(vol1_char_df['species_coord_match'] == True) & (vol1_char_df['word'].apply(is_uppercase))][["index_page_num", "word"]].drop_duplicates()

# %%
vol2_char_df[(vol2_char_df['species_coord_match'] == True) & (vol2_char_df['word'].apply(is_uppercase))][["index_page_num", "word"]].drop_duplicates()

# %%
vol3_char_df[(vol3_char_df['species_coord_match'] == True) & (vol3_char_df['word'].apply(is_uppercase))][["index_page_num", "word"]].drop_duplicates()

# %% [markdown]
# #### hardcoding Genus Platanthera

# %%
vol1_char_df[(vol1_char_df['genus_coord_match'] == True) & (vol1_char_df['word'] == 'Platan')][['word_num', 'char_num']]

# %%
vol1_char_df.shape[0]

# %%
vol1_char_df[(vol1_char_df['genus_coord_match'] == True) & (vol1_char_df['word'] == 'Platan')][['page_num', 'block_num','line_num']].iloc[0]
...
# %%
vol1_char_df.loc[(vol1_char_df['page_num'] == 631) & (vol1_char_df['block_num'] == 87) & (vol1_char_df['line_num'] == 0),'genus_coord_match'] = True
vol1_char_df.loc[(vol1_char_df['page_num'] == 631) & (vol1_char_df['block_num'] == 87) & (vol1_char_df['line_num'] == 0),'species_coord_match'] = False
...
# %%
vol1_char_df[(vol1_char_df['genus_coord_match'] == True) & (vol1_char_df['word'].apply(flag_genus))][["index_page_num", "word"]].drop_duplicates()
#skipping over all these 

# %%
not_genus_index_list_vol1 = [1730943, 1738656, 1754704]
for i in not_genus_index_list_vol1:
    vol1_char_df.loc[i : get_index_end(vol1_char_df, i),'genus_coord_match'] = False

# %%
vol1_char_df[(vol1_char_df['genus_coord_match'] == True) & (vol1_char_df['word'].apply(flag_genus))][["index_page_num", "word"]].drop_duplicates()

# %%
vol2_char_df[(vol2_char_df['genus_coord_match'] == True) & (vol2_char_df['word'].apply(flag_genus))][["index_page_num", "word"]].drop_duplicates()

# %%
not_genus_index_list_vol2 = [1939606]
for i in not_genus_index_list_vol2:
    vol2_char_df.loc[i : get_index_end(vol2_char_df, i),'genus_coord_match'] = False

# %%
vol2_char_df[(vol2_char_df['genus_coord_match'] == True) & (vol2_char_df['word'].apply(flag_genus))][["index_page_num", "word"]].drop_duplicates()

# %%
vol3_char_df[(vol3_char_df['genus_coord_match'] == True) & (vol3_char_df['word'].apply(flag_genus))][["index_page_num", "word"]].drop_duplicates()
#all okay and hybrid will get fixed later :)

# %%
vol1_char_df[(vol1_char_df['genus_coord_match'] == True) & (vol1_char_df['word'].apply(lambda x : x[0].isupper() == False))]['word']

# %%
index_apetala_correcting = vol1_char_df[(vol1_char_df['genus_coord_match'] == True) & (vol1_char_df['word'].apply(lambda x : x[0].isupper() == False))].index
vol1_char_df.loc[index_apetala_correcting,'genus_coord_match'] = False
vol1_char_df.loc[index_apetala_correcting,'species_coord_match'] = True
...
vol2_char_df[(vol2_char_df['genus_coord_match'] == True) & (vol2_char_df['word'].apply(lambda x : x[0].isupper() == False))]['word']

# %%
# fixed later when hybrids identified 
vol3_char_df[(vol3_char_df['genus_coord_match'] == True) & (vol3_char_df['word'].apply(lambda x : x[0].isupper() == False))]['word']
...
# %%
# aking sure page_num is in index
# aking sure the genus level word is not all uppercase (a family name)
# aking sure the pruned_word is not numeric (removing page_number as it's not in order usually) and removing page_num_coord_match
...
for vol_index_df in [vol1_index_df, vol2_index_df, vol3_index_df]:
    ...
    vol_index_df.loc[hybrid_genera_indecies, 'genus_coord_match'] = True
...
# %%
# f['closest_epithet_v2'] = np.nan
def extract_genus(row):
    if row['genus_coord_match'] == True:
        return row['word'] + "_" + str(row['page_num']) + "_" + str(row['block_num']) + "_" + str(row['line_num'])
    else:
        return np.nan
        
for vol_index_df in [vol1_index_df, vol2_index_df, vol3_index_df]:
    vol_index_df['closest_genus'] = vol_index_df.apply(extract_genus, axis = 1)
    vol_index_df['closest_genus'].ffill(inplace=True)

# %%
# f['closest_epithet_v2'] = np.nan
def extract_species(row):
    if row['species_coord_match'] == True:
        return row['word'] + "_" + str(row['page_num']) + "_" + str(row['block_num']) + "_" + str(row['line_num'])
    else:
        return np.nan

for vol_index_df in [vol1_index_df, vol2_index_df, vol3_index_df]:
    vol_index_df['closest_epithet'] = vol_index_df.apply(extract_species, axis = 1)
    vol_index_df.loc[vol_index_df['genus_coord_match'] == True, 'closest_epithet'] = -1
    vol_index_df['closest_epithet'].ffill(inplace=True)

# %%
def extract_infra(row):
...
for vol_index_df in [vol1_index_df, vol2_index_df, vol3_index_df]:
    vol_index_df.loc[(vol_index_df['species_coord_match'] == True) | (vol_index_df['genus_coord_match'] == True), 'closest_infra_type'] = -1
    vol_index_df['closest_infra_type'] = vol_index_df.apply(extract_infra, axis = 1)
    vol_index_df.loc[(vol_index_df['potential_infra_match'] == False) & ((vol_index_df['species_coord_match'] == True) | (vol_index_df['genus_coord_match'] == True)), 'closest_infra_type'] = -1
...
for vol_index_df in [vol1_index_df, vol2_index_df, vol3_index_df]:
    ...
    vol_index_df.loc[(vol_index_df['potential_infra_match'] == True) | (vol_index_df['species_coord_match'] == True) | (vol_index_df['genus_coord_match'] == True), 'closest_infra_name'] = -1
...
# %%
for vol_index_df in [vol1_index_df, vol2_index_df, vol3_index_df]:
    vol_index_df['potential_author_match'] = (vol_index_df['genus_coord_match'] == False) & \
                                             (vol_index_df['species_coord_match'] == False) & \
...
for vol_char_df, vol_index, doc, output_name in all_vol_data_cat_test: 
    #for each volume 
    image_list = []
    for page_num in tqdm(vol_index, desc = "Print final PDF"):
        pix_map = doc.get_page_pixmap(page_num,matrix=mat)
        image = Image.open(io.BytesIO(pix_map.tobytes()))
        draw = ImageDraw.Draw(image)
        for col_num in [0, 1]:
...
for vol_index_df in [vol1_index_df, vol2_index_df, vol3_index_df]:
    result_prune_authors = vol_index_df[(vol_index_df['genus_coord_match'] == True) |
                                        (vol_index_df['species_coord_match'] == True) |
...
if not os.path.exists(f"output{today}/index_output/"):
    os.makedirs(f"output{today}/index_output/")
...
simplified_vol1.to_csv(f'output{today}/index_output/vol1_index_output.csv')
...
simplified_vol2 = prune_authors_vol2[['closest_genus',
                                      ...
                                      'taxon_rank',
                                      'taxon_rank_detailed']]
simplified_vol2.to_csv(f'output{today}/index_output/vol2_index_output.csv')
...
simplified_vol3 = prune_authors_vol3[['closest_genus',
                                      ...
                                      'taxon_rank',
                                      'taxon_rank_detailed']]
simplified_vol3.to_csv(f'output{today}/index_output/vol3_index_output.csv')
...
non_italics_simplified_vol1 = prune_authors_vol1.loc[(prune_authors_vol1['span_flags'] != 6),
                                                     ['closest_genus',
                                                      ...
                                                      'taxon_rank',
                                                      'taxon_rank_detailed']]
non_italics_simplified_vol1.to_csv(f'output{today}/index_output/vol1_nonitalics.csv')
...
non_italics_simplified_vol2 = prune_authors_vol2.loc[(prune_authors_vol2['span_flags'] != 6),
                                                     ['closest_genus',
                                                      ...
                                                      'taxon_rank',
                                                      'taxon_rank_detailed']]
non_italics_simplified_vol2.to_csv(f'output{today}/index_output/vol2_nonitalics.csv')
...
non_italics_simplified_vol3 = prune_authors_vol3.loc[(prune_authors_vol3['span_flags'] != 6),
                                                     ['closest_genus',
                                                      ...
                                                      'taxon_rank',
                                                      'taxon_rank_detailed']]
non_italics_simplified_vol3.to_csv(f'output{today}/index_output/vol3_nonitalics.csv')

print("THE END: Proceed to MainText")
