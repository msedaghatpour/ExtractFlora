...
vol2_last8_path = '../input/nouvelle flore_Vol2_index R-Z_OCR.pdf'
...
def extract_text(vol, page, page_num, word_list):
    """ DOCUMENTATION
...
def char_df(vol, pages):
    word_list = []
    for page_num in tqdm(range(len(pages)), desc = "getting characters"):
        page = pages[page_num]
        extract_text(vol, page, page_num, word_list)
    return pd.DataFrame(word_list)
# ---------------------------------------------------------------- #


# ------------------------- Process words ------------------------ #
def process_words(vol_df, drop_coords = True):
...
# ----------------------- Extract And Save ----------------------- #
print("\nextracting volume 1")
vol1_df = char_df("1", vol1_pages)
process_words(vol1_df)
vol1_df = rearrange_cols(vol1_df)

print("\nextracting volume 2")
vol2_df = char_df("2", vol2_pages)
process_words(vol2_df)
vol2_df = rearrange_cols(vol2_df)

print("\nextracting volume 3")
vol3_df = char_df("3", vol3_pages)
process_words(vol3_df)
vol3_df = rearrange_cols(vol3_df)

# saving
if not os.path.exists(f"output{today}/char_df"):
    os.makedirs(f"output{today}/char_df")

print("\n\nSaving volume 1")
vol1_df.to_pickle(f"output{today}/char_df/vol1_df.pkl")

print("Saving volume 2")
vol2_df.to_pickle(f"output{today}/char_df/vol2_df.pkl")

print("Saving volume 3")
vol3_df.to_pickle(f"output{today}/char_df/vol3_df.pkl")
print("THE END: Proceed to index")
# ---------------------------------------------------------------- #
