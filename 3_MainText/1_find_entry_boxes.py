# ExtractFlora/MainText script 1
# get n grams
...
# rename of columns of mout. indecies 
...
# extracts genus and species names 
vol_index_lists = []
...
#       closest_match_score is also calculated AFTER closest_match is used
#                                   meaning depending on how difflib.get_close_matches works
#                                   a potential match with a high closest_match_score
#                                   score won't be picked up.
def closest_match(input_str, matching_lst):
    """ DOCUMENTATION
...
def closest_match_score(input_str, match_str):
    """ DOCUMENTATION
...
# main code. fuzzy match index to main text
i = 1
for vol_word_df, vol_species in [(vol1_word_df, vol_index_lists[0][1]), (vol2_word_df, vol_index_lists[1][1]), (vol3_word_df, vol_index_lists[2][1])]:
    ...
    for n in range(1, 5):
        ...
        vol_word_df[match_col_name] = vol_word_df[word_group_col_name].progress_apply(lambda x: closest_match(x, vol_species))
        vol_word_df[match_score_col_name] = vol_word_df.progress_apply(lambda r: closest_match_score(r[word_group_col_name], r[match_col_name]), axis = 1)
    i += 1 
...
# LOOP
# Identifying potential binomial lines,
# Determining section boundaries,
# Calculating section bounding boxes,
# Marking binomial sections,
...
for vol_word_df, vol_genera in [(vol1_word_df, vol_index_lists[0][0]), (vol2_word_df, vol_index_lists[1][0]), (vol3_word_df, vol_index_lists[2][0])]:
    ...
    # save
    if not os.path.exists(f"{output_dir}/desc_box_df"):
        os.makedirs(f"{output_dir}/desc_box_df")
    vol_word_df.to_pickle(f"{output_dir}/desc_box_df/vol{i}_entry_df.pkl")
    i += 1
    # ---------------------------------------------------------------- #
